...
namespace HeaderLinkedListProject
{
    class HeaderLinkedList
    {
        ...
        public void InsertBefore(int data, int x)
        {
            ...
            /*Find the pointer to predecessor of node containing x */
            Node p = head;
            while(p.link != null)
            {
                if (p.link.info == x)
        ...
        public void InsertAtPosition(int data, int k)
        {
            ...
            if (p == null)
                Console.WriteLine("You can insert only up to " + (i-1) + "th positions.");
            else
            {
                temp = new Node(data);
                temp.link = p.link;
        ...
        public void Reverse() /* Last element issue */
        {
            Node prev, p, next;
            prev = null;
            p = head;
